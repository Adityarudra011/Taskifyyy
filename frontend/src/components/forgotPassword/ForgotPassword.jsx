    ...
    return (
        <div className="text-center">
            ...
            <form className="w-2/5 mx-auto p-5" onSubmit={handleSubmit}>
                <input type="email"
                    className="p-3 rounded-2xl shadow-lg w-full"
                    placeholder="Enter your email"
                    onChange={(e) => setEmail(e.target.value)}
                    value={email}
                    required
                />
                <button className="p-2 rounded-full shadow-md bg-indigo-700 text-white px-5 mt-10 disabled:bg-indigo-500 transform transition-transform duration-300 hover:scale-110" disabled={isLoading}>Reset</button>
            </form>
            {
